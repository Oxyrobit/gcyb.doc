...
```bash
# Affiche les informations détaillées de la clé
openssl rsa -in private/ACR_CYBER.key -noout -text
# Affiche la clé publique
openssl rsa -in private/ACR_CYBER.key -pubout
```
...
**le modèle** doit contenir `basicConstraints = critical, CA:true`
...
```bash
# Affiche les informations détaillées du certificat (on peux y voir la duré de validité)
openssl x509 -in crt/ACR_CYBER.crt -noout -text
# Afficher les informations sur l’utilisation possible du certificat
openssl x509 -purpose -in crt/ACR_CYBER.crt
```
...
```bash
openssl genrsa -out private/ACI_SERVERS.key -rand private/.rand 4096
```
...
### Client: Génération du certificat client
